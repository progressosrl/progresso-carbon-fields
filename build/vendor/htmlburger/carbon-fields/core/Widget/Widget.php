...
namespace Progresso\Carbon_Fields\Widget;
 ...
 */
abstract class Widget extends \Progresso\WP_Widget
{
     ...
     * @var string
     */
    protected $widget_id_prefix = 'progresso_carbon_fields_';
    /**
     * Create the widget.
    ...
    public function setup($widget_id, $title, $description, $fields, $classname = '')
    {
        ...
        $this->datastore = \Progresso\Carbon_Fields\Datastore\Datastore::make('widget');
        if (empty($title)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Empty widget title is not supported');
        }
    ...
    public function update($new_instance, $old_instance)
    {
        ...
        $new_instance = \Progresso\Carbon_Fields\Helper\Helper::expand_compacted_input($new_instance);
    ...
    public function form($instance)
    {
        ...
        \Progresso\Carbon_Fields\Container\Container::factory('widget', $this->id, $this->id)->add_fields($custom_fields)->init();
    }
    /**
    ...
    public function register_widget_id($id)
    {
        if (\in_array($id, static::$registered_widget_ids)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Widget with ID "' . $id . '" already registered. Please change the widget title');
            return;
        }
