...
use Progresso\WP_Query;
use Progresso\WP_Term_Query;
use Progresso\WP_User_Query;
use Progresso\WP_Comment_Query;
/**
 ...
 *  - Comments
 */
class Association_Field extends \Progresso\Carbon_Fields\Field\Field
{
    /**
    ...
    {
        $this->wp_toolset = \Progresso\Carbon_Fields\Carbon_Fields::resolve('wp_toolset');
        $this->set_value_set(new \Progresso\Carbon_Fields\Value_Set\Value_Set(\Progresso\Carbon_Fields\Value_Set\Value_Set::TYPE_VALUE_SET, array('type' => '', 'subtype' => '', 'id' => 0)));
    ...
    protected function value_string_to_property_array($value_string)
    {
        ...
        $property_array = array(\Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY => $value_string, 'type' => $type, 'subtype' => $subtype, 'id' => \intval($id));
    ...
    protected function value_string_array_to_value_set($value_string_array)
    {
        ...
        foreach ($value_string_array as $raw_value_entry) {
            ...
            if (\is_array($raw_value_entry)) {
                ...
                $value_string = $raw_value_entry[\Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY];
            }
    ...
    public function get_options($args = array())
    {
         ...
         */
        $options = apply_filters('progresso_carbon_fields_association_field_options', $options, $this->get_base_name());
    ...
    public function get_post_options_sql($args = array())
    {
         ...
         */
        $filter_name = 'progresso_carbon_fields_association_field_options_' . $this->get_base_name() . '_' . $type . '_' . $post_type;
        ...
        $posts_query = new \Progresso\WP_Query($args);
    ...
    public function get_term_options_sql($args = array())
    {
         ...
         */
        $filter_name = 'progresso_carbon_fields_association_field_options_' . $this->get_base_name() . '_' . $type . '_' . $taxonomy;
        ...
        $terms_query = new \Progresso\WP_Term_Query($args);
    ...
    public function get_user_options_sql($args = array())
    {
         ...
         */
        $filter_name = 'progresso_carbon_fields_association_field_options_' . $this->get_base_name() . '_' . $type;
        $args = apply_filters($filter_name, array('fields' => 'ID', 'number' => 1, 'search' => $search_term));
        $users_query = new \Progresso\WP_User_Query();
    ...
    public function get_comment_options_sql($args = array())
    {
        ...
        $comments_query = new \Progresso\WP_Comment_Query();
     ...
     *
     * Can be overriden or extended by the `progresso_carbon_fields_association_field_option_thumbnail` filter.
     *
    ...
    public function get_thumbnail_by_type($id, $type, $subtype = '')
    {
        ...
        return apply_filters('progresso_carbon_fields_association_field_option_thumbnail', $thumbnail_url, $id, $type, $subtype);
    }
    /**
    ...
    public function get_title_by_type($id, $type, $subtype = '')
    {
        ...
        if ($type === 'comment') {
            $max = apply_filters('progresso_carbon_fields_association_field_comment_length', 30, $this->get_base_name());
            if (\strlen($title) > $max) {
         ...
         */
        $title = apply_filters('progresso_carbon_fields_association_field_title', $title, $this->get_base_name(), $id, $type, $subtype);
        if (!$title) {
    ...
    public function get_item_label($id, $type, $subtype = '')
    {
         ...
         * @param string $subtype Subtype - "page", "post", "category", etc.
         */
        return apply_filters('progresso_carbon_fields_association_field_item_label', $label, $this->get_base_name(), $id, $type, $subtype);
    }
    /**
