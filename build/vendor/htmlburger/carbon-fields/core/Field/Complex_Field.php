 ...
 */
class Complex_Field extends \Progresso\Carbon_Fields\Field\Field
{
    /**
    ...
    {
        $this->set_value_set(new \Progresso\Carbon_Fields\Value_Set\Value_Set(\Progresso\Carbon_Fields\Value_Set\Value_Set::TYPE_MULTIPLE_VALUES));
     ...
     * @param boolean             $set_as_default
     */
    protected function update_child_datastore(\Progresso\Carbon_Fields\Datastore\Datastore_Interface $datastore, $set_as_default = \false)
    {
    ...
    public function add_fields()
    {
        ...
        $name = !empty($name) ? $name : \Progresso\Carbon_Fields\Field\Group_Field::DEFAULT_GROUP_NAME;
        if (\array_key_exists($name, $this->groups)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Group with name "' . $name . '" in Complex Field "' . $this->get_label() . '" already exists.');
            return $this;
        }
        $reserved_names = array(\Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY, static::TYPE_PROPERTY);
        foreach ($fields as $field) {
            /** @var Field $field */
            if (\in_array($field->get_base_name(), $reserved_names)) {
                \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('"' . $field->get_base_name() . '" is a reserved keyword for Complex fields and cannot be used for a field name.');
                return $this;
            }
        }
        $group = new \Progresso\Carbon_Fields\Field\Group_Field($name, $label, $fields);
    ...
    protected function get_prefilled_groups($groups, $value_tree)
    {
        ...
        foreach ($value_tree as $group_index => $value) {
            ...
            $fields[$group_index] = array(\Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY => $group->get_name()) + $this->get_prefilled_group_fields($group_fields, $group_values, $group_index);
        }
        return $fields;
    }
    /**
    ...
    public function set_value_from_input($input)
    {
        ...
        foreach ($input_groups as $values) {
            if (!isset($values[\Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY]) || !isset($this->groups[$values[\Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY]])) {
                continue;
            }
            $group = $this->get_group_by_name($values[\Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY]);
            $group_fields = $group->get_fields();
            $group_field_names = \array_flip($group->get_field_names());
            $value_group = array(\Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY => $values[\Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY]);
            unset($values[\Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY]);
            ...
            foreach ($group_fields as $field) {
                ...
                if ($tmp_field instanceof \Progresso\Carbon_Fields\Field\Complex_Field) {
    ...
    public function save()
    {
        ...
        $delete_on_save = apply_filters('progresso_carbon_fields_should_delete_field_value_on_save', $delete_on_save, $this);
        if ($delete_on_save) {
            $this->delete();
        }
        $save = apply_filters('progresso_carbon_fields_should_save_field_value', \true, $this->get_value(), $this);
        if ($save) {
            $this->get_datastore()->save(apply_filters('progresso_carbon_fields_before_complex_field_save', $this));
        ...
        foreach ($field_groups as $group_index => $field_group) {
            ...
            foreach ($field_group as $key => $field) {
                if ($field instanceof \Progresso\Carbon_Fields\Field\Field) {
                    ...
                    if ($key === \Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY) {
    ...
    protected function external_to_internal_value($value)
    {
        ...
        $value[\Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY] = $value[static::TYPE_PROPERTY];
    ...
    public function set_value($value)
    {
        ...
        foreach ($value as $values) {
            $groups[] = isset($values[\Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY]) ? $values[\Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY] : \Progresso\Carbon_Fields\Field\Group_Field::DEFAULT_GROUP_NAME;
        }
    ...
    public function set_default_value($default_value)
    {
        foreach ($default_value as $index => $group) {
            if (!isset($group[static::TYPE_PROPERTY])) {
                $default_value[$index][static::TYPE_PROPERTY] = \Progresso\Carbon_Fields\Field\Group_Field::DEFAULT_GROUP_NAME;
            }
        }
    ...
    public function to_json($load)
    {
        ...
        $value_data = array();
        foreach ($field_groups as $group_index => $fields) {
            $group = $this->get_group_by_name($fields[\Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY]);
    ...
    public function set_layout($layout)
    {
        ...
        if (!\in_array($layout, $available_layouts)) {
            ...
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise($error_message);
            return $this;
        }
        $this->layout = $layout;
        return $this;
