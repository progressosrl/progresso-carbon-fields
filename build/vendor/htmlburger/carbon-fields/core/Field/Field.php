...
use Progresso\Carbon_Fields\Value_Set\Value_Set;
 ...
 */
class Field implements \Progresso\Carbon_Fields\Datastore\Datastore_Holder_Interface
{
    /**
    ...
    public static function factory($raw_type, $name, $label = null)
    {
        ...
        // @see Field::set_name()
        if (!\Progresso\Carbon_Fields\Helper\Helper::is_valid_entity_id($name)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Field names can only contain lowercase alphanumeric characters, dashes and underscores ("' . $name . '" passed).');
            return null;
        }
        if (\Progresso\Carbon_Fields\Carbon_Fields::has($type, 'fields')) {
            return \Progresso\Carbon_Fields\Carbon_Fields::resolve_with_arguments($type, array('type' => $type, 'name' => $name, 'label' => $label), 'fields');
        }
        // Fallback to class name-based resolution
        $class = \Progresso\Carbon_Fields\Helper\Helper::type_to_class($type, __NAMESPACE__, '_Field');
        if (!\class_exists($class)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Unknown field type "' . $raw_type . '".');
    ...
    {
        return \Progresso\Carbon_Fields\Helper\Helper::class_to_type(\get_class($this), '_Field');
    }
    /**
    ...
    public function activate()
    {
        ...
        do_action('progresso_carbon_fields_field_activated', $this);
    }
    /**
    ...
    public function is_simple_root_field()
    {
        ...
        return empty($hierarchy) && \in_array($this->get_value_set()->get_type(), array(\Progresso\Carbon_Fields\Value_Set\Value_Set::TYPE_SINGLE_VALUE, \Progresso\Carbon_Fields\Value_Set\Value_Set::TYPE_MULTIPLE_PROPERTIES));
    }
    /**
    ...
    {
        $delete_on_save = apply_filters('progresso_carbon_fields_should_delete_field_value_on_save', \true, $this);
        if ($delete_on_save) {
            $this->delete();
        }
        $save = apply_filters('progresso_carbon_fields_should_save_field_value', \true, $this->get_value(), $this);
        if ($save) {
            $this->get_datastore()->save(apply_filters('progresso_carbon_fields_before_field_save', $this));
        }
    }
    /**
    ...
    {
        $this->get_datastore()->delete(apply_filters('progresso_carbon_fields_before_field_delete', $this));
    }
    /**
    ...
    public function get_value_set()
    {
        if ($this->value_set === null) {
            $this->set_value_set(new \Progresso\Carbon_Fields\Value_Set\Value_Set());
        }
    ...
    {
        if (empty($name)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Field name can\'t be empty');
            return $this;
        }
    ...
    public function set_label($label)
    {
        if (\is_null($label)) {
            ...
            $label = \Progresso\Carbon_Fields\Helper\Helper::normalize_label($this->get_name());
        }
        $this->label = $label;
        return $this;
    }
    ...
    public function set_attribute($name, $value = '')
    {
        ...
        if (!$is_data_attribute && !\in_array($name, $this->allowed_attributes)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Only the following attributes are allowed: ' . \implode(', ', \array_merge($this->allowed_attributes, array('data-*'))));
            return $this;
        }
    ...
    public function set_attributes($attributes)
    {
        if (!\is_array($attributes)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('An array must be passed for the $attributes parameter of Field::set_attributes().');
            return $this;
        }
    ...
    protected function parse_conditional_rule($rule)
    {
        ...
        if (!\is_array($rule) || empty($rule['field'])) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Invalid conditional logic rule format. The rule should be an array with the "field" key set.');
            return null;
        }
        // Fill in optional keys with defaults
        $rule = \array_merge(array('compare' => '=', 'value' => ''), $rule);
        if (!\in_array($rule['compare'], $allowed_operators)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Invalid conditional logic compare operator: <code>' . $rule['compare'] . '</code><br>Allowed operators are: <code>' . \implode(', ', $allowed_operators) . '</code>');
            return null;
        }
        if (\in_array($rule['compare'], $array_operators) && !\is_array($rule['value'])) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Invalid conditional logic value format. An array is expected, when using the "' . $rule['compare'] . '" operator.');
            return null;
        }
        return $rule;
    }
    ...
    {
        if (!\is_array($rules)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Conditional logic rules argument should be an array.');
            return array();
        }
        $parsed_rules = array('relation' => \Progresso\Carbon_Fields\Helper\Helper::get_relation_type_from_array($rules), 'rules' => array());
        $rules_only = $rules;
