...
use Progresso\Carbon_Fields\Container\Container;
 ...
 */
class Legacy_Storage_Service_v_1_5 extends \Progresso\Carbon_Fields\Service\Service
{
    /**
     ...
     */
    public function __construct(\Progresso\Carbon_Fields\Container\Repository $container_repository, \Progresso\Carbon_Fields\Toolset\Key_Toolset $key_toolset)
    {
    ...
    {
        add_filter('progresso_carbon_fields_datastore_storage_array', array($this, 'filter_storage_array'), 10, 3);
    }
    /**
     * Disable the service
     */
    ...
    {
        remove_filter('progresso_carbon_fields_datastore_storage_array', array($this, 'filter_storage_array'), 10);
    }
    /**
     ...
     * @return Container
     */
    protected function get_container_for_datastore(\Progresso\Carbon_Fields\Datastore\Datastore_Interface $datastore)
    {
    ...
    protected function get_field_group_permutations($fields)
    {
        ...
        foreach ($fields as $field) {
            if ($field instanceof \Progresso\Carbon_Fields\Field\Complex_Field) {
     ...
     * @return array
     */
    protected function get_table_details_for_datastore(\Progresso\Carbon_Fields\Datastore\Datastore_Interface $datastore)
    {
        global $wpdb;
        $details = array('prefix' => '_', 'table_name' => '', 'table_id_column' => '', 'table_key_column' => '', 'table_value_column' => '');
        if ($datastore instanceof \Progresso\Carbon_Fields\Datastore\Theme_Options_Datastore) {
            $details['prefix'] = '';
            $details['table_name'] = $wpdb->options;
            $details['table_key_column'] = 'option_name';
            $details['table_value_column'] = 'option_value';
        } else {
            if ($datastore instanceof \Progresso\Carbon_Fields\Datastore\Meta_Datastore) {
     ...
     * @return array<string>
     */
    protected function get_legacy_sql_comparisons_for_field(\Progresso\Carbon_Fields\Field\Field $field, $key_prefix, $key_column)
    {
        ...
        if ($field instanceof \Progresso\Carbon_Fields\Field\Map_Field) {
     ...
     * @return array
     */
    protected function get_legacy_storage_array_from_database(\Progresso\Carbon_Fields\Container\Container $container, \Progresso\Carbon_Fields\Datastore\Datastore_Interface $datastore)
    {
        global $wpdb;
     ...
     * @return array
     */
    public function get_legacy_storage_array(\Progresso\Carbon_Fields\Datastore\Datastore_Interface $datastore)
    {
    ...
    protected function get_value_for_legacy_key($key, $legacy_storage_array)
    {
        ...
        if (isset($legacy_storage_array[$key . '-' . $first_map_key])) {
            $value = array(\Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY => $value);
            foreach ($this->map_keys as $map_key) {
    ...
    protected function row_descriptor_to_storage_array($row_descriptor)
    {
        ...
        foreach ($complex_parents as $level => $complex_parent) {
            ...
            $key = $this->key_toolset->get_storage_key(\false, \array_slice($hierarchy, 0, $level + 1), $complex_parent_hierarchy_index, $complex_parent_value_index, \Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY);
        ...
        if (\is_array($unserialized_value)) {
            if (isset($unserialized_value[\Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY])) {
                ...
                foreach ($unserialized_value as $value_item) {
                    $key = $this->key_toolset->get_storage_key(\false, $hierarchy, $hierarchy_index, $i, \Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY);
                    $storage_array[$key] = $value_item;
                    $i++;
                }
            }
            ...
            if ($unserialized_value === null) {
                ...
                $key = $this->key_toolset->get_storage_key(\false, $hierarchy, $hierarchy_index, 0, \Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY);
                $storage_array[$key] = $row_descriptor['value'];
            }
        }
     ...
     * @return array
     */
    public function get_storage_array_for_datastore(\Progresso\Carbon_Fields\Datastore\Datastore_Interface $datastore)
    {
     ...
     * @return array
     */
    public function get_storage_array(\Progresso\Carbon_Fields\Datastore\Datastore_Interface $datastore, $storage_key_patterns)
    {
