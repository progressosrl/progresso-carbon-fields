 ...
 */
abstract class Datastore implements \Progresso\Carbon_Fields\Datastore\Datastore_Interface
{
    /**
    ...
    public static function factory($raw_type)
    {
        ...
        $class = \Progresso\Carbon_Fields\Helper\Helper::type_to_class($type, __NAMESPACE__, '_Datastore');
        if (!\class_exists($class)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Unknown datastore type "' . $raw_type . '".');
            return null;
        }
        $datastore = new $class();
        return $datastore;
