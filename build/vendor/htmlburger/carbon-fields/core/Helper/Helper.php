...
namespace Progresso\Carbon_Fields\Helper;

use Progresso\Carbon_Fields\Datastore\Datastore;
...
class Helper
{
    ...
    {
        \Progresso\Carbon_Fields\Carbon_Fields::verify_fields_registered();
    ...
    public static function set_value($object_id, $container_type, $container_id, $field_name, $value)
    {
        static::with_field_clone($object_id, $container_type, $container_id, $field_name, function ($field) use($container_type, $container_id, $field_name, $value) {
            if (!$field) {
                ...
                \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Could not find a field which satisfies the supplied pattern ' . $container_message . ': ' . $field_name);
                return;
            }
            /** @var \Carbon_Fields\Field\Field $field */
    ...
    public static function get_relation_type_from_array($array, $allowed_relations = array('AND', 'OR'), $relation_key = 'relation')
    {
        ...
        if (!\in_array($relation, $allowed_relations)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Invalid relation type ' . $relation . '. ' . 'The rule should be one of the following: "' . \implode('", "', $allowed_relations) . '"');
        }
        return $relation;
    }
    /**
    ...
    public static function get_attachment_id($url)
    {
         ...
         * @param string $url
         */
        $url = apply_filters('progresso_carbon_fields_attachment_id_base_url', $url);
        ...
        if (\strpos($url, $dir['baseurl'] . '/') !== \false) {
            ...
            $query = new \Progresso\WP_Query($query_args);
            if ($query->have_posts()) {
         ...
         * @param  string  $url
         */
        return apply_filters('progresso_carbon_fields_attachment_id_from_url', $attachment_id, $url);
    }
    /**
    ...
    public static function get_attachment_metadata($id, $type)
    {
        ...
        if (!$attachment) {
             ...
             */
            return apply_filters('progresso_carbon_fields_attachment_not_found_metadata', $attachment_metadata, $id, $type);
        }
         ...
         * @param string          $type                 The type of `$id` passed. Either 'id' or 'url'.
         */
        return apply_filters('progresso_carbon_fields_attachment_metadata', $attachment_metadata, $id, $type);
    }
    /**
    ...
    public static function input()
    {
        ...
        if (\Progresso\Carbon_Fields\COMPACT_INPUT) {
    ...
    {
        if (isset($input[\Progresso\Carbon_Fields\COMPACT_INPUT_KEY])) {
            $inputs = $input[\Progresso\Carbon_Fields\COMPACT_INPUT_KEY];
