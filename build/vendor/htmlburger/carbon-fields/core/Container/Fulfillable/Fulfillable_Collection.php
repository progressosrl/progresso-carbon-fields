...
namespace Progresso\Carbon_Fields\Container\Fulfillable;

use Progresso\Carbon_Fields\Container\Condition\Factory;
use Progresso\Carbon_Fields\Container\Fulfillable\Translator\Array_Translator;
use Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception;
class Fulfillable_Collection implements \Progresso\Carbon_Fields\Container\Fulfillable\Fulfillable
{
    /**
     ...
     */
    public function __construct(\Progresso\Carbon_Fields\Container\Condition\Factory $condition_factory, \Progresso\Carbon_Fields\Container\Fulfillable\Translator\Array_Translator $array_translator)
    {
    ...
    {
        return \Progresso\Carbon_Fields\Carbon_Fields::resolve('container_condition_fulfillable_collection');
    }
    /**
    ...
    protected function propagate_condition_type_list()
    {
        ...
        foreach ($fulfillables as $fulfillable) {
            if ($fulfillable['fulfillable'] instanceof \Progresso\Carbon_Fields\Container\Fulfillable\Fulfillable_Collection) {
    ...
    public function where($condition_type, $comparison_operator = '=', $value = null, $fulfillable_comparison = 'AND')
    {
        ...
        if (!$this->is_condition_type_allowed($condition_type)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Unsupported container condition used: ' . $condition_type);
            return $this;
        }
        if ($value === null) {
     ...
     */
    public function add_fulfillable(\Progresso\Carbon_Fields\Container\Fulfillable\Fulfillable $fulfillable, $fulfillable_comparison)
    {
        if (!\in_array($fulfillable_comparison, $this->supported_fulfillable_comparisons)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Invalid fulfillable comparison passed: ' . $fulfillable_comparison);
            return;
        }
     ...
     */
    public function remove_fulfillable(\Progresso\Carbon_Fields\Container\Fulfillable\Fulfillable $fulfillable)
    {
        $fulfillables = $this->get_fulfillables();
