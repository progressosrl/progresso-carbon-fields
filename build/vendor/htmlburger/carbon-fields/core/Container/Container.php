...
namespace Progresso\Carbon_Fields\Container;

use Progresso\Carbon_Fields\Carbon_Fields;
use Progresso\Carbon_Fields\Helper\Helper;
use Progresso\Carbon_Fields\Field\Field;
use Progresso\Carbon_Fields\Field\Group_Field;
use Progresso\Carbon_Fields\Container\Fulfillable\Fulfillable_Collection;
use Progresso\Carbon_Fields\Datastore\Datastore_Interface;
use Progresso\Carbon_Fields\Datastore\Datastore_Holder_Interface;
use Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception;
/**
 * Base container class.
 * Defines the key container methods and their default implementations.
 */
abstract class Container implements \Progresso\Carbon_Fields\Datastore\Datastore_Holder_Interface
{
    /**
    ...
    public static function factory($raw_type, $id, $name = '')
    {
        ...
        $type = \Progresso\Carbon_Fields\Helper\Helper::normalize_type($raw_type);
        $repository = \Progresso\Carbon_Fields\Carbon_Fields::resolve('container_repository');
        $id = $repository->get_unique_container_id($id !== '' ? $id : $name);
        if (!\Progresso\Carbon_Fields\Helper\Helper::is_valid_entity_id($id)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Container IDs can only contain lowercase alphanumeric characters, dashes and underscores ("' . $id . '" passed).');
            return null;
        }
        if (!$repository->is_unique_container_id($id)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('The passed container id is already taken ("' . $id . '" passed).');
            return null;
        }
        $container = null;
        if (\Progresso\Carbon_Fields\Carbon_Fields::has($type, 'containers')) {
            $container = \Progresso\Carbon_Fields\Carbon_Fields::resolve_with_arguments($type, array('id' => $id, 'name' => $name, 'type' => $type), 'containers');
        } else {
            // Fallback to class name-based resolution
            $class = \Progresso\Carbon_Fields\Helper\Helper::type_to_class($type, __NAMESPACE__, '_Container');
            if (!\class_exists($class)) {
                \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Unknown container "' . $raw_type . '".');
                $class = __NAMESPACE__ . '\\Broken_Container';
            }
            $fulfillable_collection = \Progresso\Carbon_Fields\Carbon_Fields::resolve('container_condition_fulfillable_collection');
            $condition_translator = \Progresso\Carbon_Fields\Carbon_Fields::resolve('container_condition_translator_json');
    ...
    {
        \Progresso\Carbon_Fields\Carbon_Fields::verify_boot();
        if (empty($title)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Empty container title is not supported');
        }
        $this->id = $id;
        $this->title = $title;
        $this->type = $type;
    ...
    protected function get_condition_types($static)
    {
        ...
        $container_type = \Progresso\Carbon_Fields\Helper\Helper::class_to_type(\get_class($this), '_Container');
        $condition_types = array();
        $condition_types = apply_filters('progresso_carbon_fields_' . $container_type . '_container_' . $group . '_condition_types', $condition_types, $container_type, $this);
        $condition_types = apply_filters('progresso_carbon_fields_container_' . $group . '_condition_types', $condition_types, $container_type, $this);
    ...
    protected function activate()
    {
        $this->active = \true;
        $this->boot();
        do_action('progresso_carbon_fields_container_activated', $this);
    ...
    {
        foreach ($this->fields as $field) {
            $field->set_value_from_input(\Progresso\Carbon_Fields\Helper\Helper::input());
            $field->save();
        }
    }
    /**
    ...
    protected final function _is_valid_save()
    {
        ...
        return apply_filters('progresso_carbon_fields_container_is_valid_save', $is_valid_save, $this);
    }
    /**
     * Checks whether the current save request is valid
     *
    ...
    public final function is_valid_attach()
    {
        ...
        return apply_filters('progresso_carbon_fields_container_is_valid_attach', $is_valid_attach, $this);
    }
    /**
    ...
    {
        $field_name_characters = \Progresso\Carbon_Fields\Helper\Helper::get_field_name_characters_pattern();
        // matches:
        // field_name
        // field_name[0]
    ...
    public function get_field_by_name($field_name)
    {
        ...
        while (!empty($hierarchy_left)) {
            ...
            $segment_group_name = isset($segment_pieces['group_name']) ? $segment_pieces['group_name'] : \Progresso\Carbon_Fields\Field\Group_Field::DEFAULT_GROUP_NAME;
            foreach ($field_group as $f) {
                ...
                if (empty($hierarchy_left)) {
                    ...
                    if (!$f instanceof \Progresso\Carbon_Fields\Field\Complex_Field) {
                        return null;
                    }
    ...
    protected function register_field_name($name)
    {
        if (\in_array($name, $this->registered_field_names)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Field name "' . $name . '" already registered');
            return \false;
        }
     ...
     */
    public function set_datastore(\Progresso\Carbon_Fields\Datastore\Datastore_Interface $datastore, $set_as_default = \false)
    {
    ...
    {
        $input = \Progresso\Carbon_Fields\Helper\Helper::input();
    ...
    private function create_tab($tab_name, $fields, $queue_end = self::TABS_TAIL)
    {
        if (isset($this->tabs[$tab_name])) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise("Tab name duplication for {$tab_name}");
        }
    ...
    protected function get_tabs()
    {
        ...
        if (!empty($untabbed_fields)) {
            $this->create_tab(apply_filters('progresso_carbon_fields_untabbed_fields_tab_title', __('General', 'carbon-fields'), $this), $untabbed_fields, static::TABS_HEAD);
        }
        return $this->tabs;
    }
    /**
    ...
    {
        $this->classes = \Progresso\Carbon_Fields\Helper\Helper::sanitize_classes($classes);
        return $this;
    }
    /**
    ...
    {
        foreach ($fields as $field) {
            if (!$field instanceof \Progresso\Carbon_Fields\Field\Field) {
                \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Progresso\\Object must be of type Carbon_Fields\\Field\\Field');
                return $this;
            }
