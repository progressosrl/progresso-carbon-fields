 ...
 */
class Theme_Options_Container extends \Progresso\Carbon_Fields\Container\Container
{
    /**
    ...
    public function __construct($id, $title, $type, $condition_collection, $condition_translator)
    {
        ...
        if (!$this->get_datastore()) {
            $this->set_datastore(\Progresso\Carbon_Fields\Datastore\Datastore::make('theme_options'), $this->has_default_datastore());
        }
        if (apply_filters('progresso_carbon_fields_' . $type . '_container_admin_only_access', \true, $title, $this)) {
    ...
    public function save($user_data = null)
    {
        ...
        do_action('progresso_carbon_fields_' . $this->type . '_container_saved', $user_data, $this);
        if (!\headers_sent()) {
    ...
    public function render()
    {
        ...
        include \Progresso\Carbon_Fields\DIR . '/templates/Container/' . $this->type . '.php';
    }
    /**
    ...
    protected function register_page()
    {
        ...
        if (!$parent) {
            ...
            if (isset(static::$registered_pages[$file])) {
                \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Page "' . $file . '" already registered');
                return \false;
            }
        ...
        if (\in_array($file, static::$registered_pages[$parent])) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Page "' . $file . '" with parent "' . $parent . '" is already registered. Please set a name for the container.');
            return \false;
        }
