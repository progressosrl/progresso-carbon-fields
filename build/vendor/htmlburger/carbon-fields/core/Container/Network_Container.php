 ...
 */
class Network_Container extends \Progresso\Carbon_Fields\Container\Theme_Options_Container
{
    /**
    ...
    public function __construct($id, $title, $type, $condition_collection, $condition_translator)
    {
        ...
        if (!is_multisite()) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('The "' . $title . '" container will not be available because your site is not a multisite.');
            return;
        }
        $this->set_datastore(\Progresso\Carbon_Fields\Datastore\Datastore::make('network'), $this->has_default_datastore());
    ...
    {
        if (!\function_exists('Progresso\\get_blog_status')) {
            return \false;
        }
    ...
    public function set_site_id($id)
    {
        ...
        if (!$this->site_exists($id)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('The specified site id #' . $id . ' does not exist');
            return $this;
        }
        $this->site_id = $id;
