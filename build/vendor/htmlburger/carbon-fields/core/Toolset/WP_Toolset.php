...
class WP_Toolset
{
    ...
    public function get_term_by_descriptor($term_descriptor)
    {
        if (!\is_array($term_descriptor) || !isset($term_descriptor['value']) || !isset($term_descriptor['taxonomy'])) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Term descriptor passed is invalid. Please supply an array with a "value" and a "taxonomy" key: ' . \print_r($term_descriptor, \true));
            return null;
        }
        ...
        if (!$term) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Failed to load term for descriptor: ' . \print_r($term_descriptor, \true));
            return new \Progresso\WP_Term(new \stdClass());
        }
        return $term;
    }
    /**
