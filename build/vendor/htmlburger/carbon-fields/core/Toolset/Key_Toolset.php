...
namespace Progresso\Carbon_Fields\Toolset;
...
class Key_Toolset
{
    ...
    {
        $full_hierarchy_index = $this->get_sanitized_hierarchy_index($full_hierarchy, $full_hierarchy_index);
        if ($is_simple_root_field && $property === \Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY) {
     ...
     * @return string
     */
    public function get_storage_key_with_index_wildcards($is_simple_root_field, $full_hierarchy, $property = \Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY, $wildcard = '%')
    {
        if ($is_simple_root_field && $property === \Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY) {
            return $this->get_storage_key_for_simple_root_field(\array_shift($full_hierarchy));
        }
        ...
        foreach ($patterns as $storage_key => $type) {
            ...
            switch ($type) {
                ...
                case static::PATTERN_COMPARISON_STARTS_WITH:
                    ...
                    break;
                default:
                    \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Unsupported storage key pattern type used: "' . $type . '"');
                    break;
            }
            $sql[] = $comparison;
        }
    ...
    {
        $parsed = array('root' => '', 'hierarchy' => array(), 'hierarchy_index' => array(), 'value_index' => 0, 'property' => \Progresso\Carbon_Fields\Value_Set\Value_Set::VALUE_PROPERTY);
