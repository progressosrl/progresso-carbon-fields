...
namespace Progresso\Carbon_Fields;

use Progresso\Carbon_Fields\Pimple\Container as PimpleContainer;
use Progresso\Carbon_Fields\Helper\Helper;
use Progresso\Carbon_Fields\Loader\Loader;
use Progresso\Carbon_Fields\Container\Repository as ContainerRepository;
use Progresso\Carbon_Fields\Toolset\Key_Toolset;
use Progresso\Carbon_Fields\Toolset\WP_Toolset;
use Progresso\Carbon_Fields\Service\Meta_Query_Service;
use Progresso\Carbon_Fields\Service\Legacy_Storage_Service_v_1_5;
use Progresso\Carbon_Fields\Service\Revisions_Service;
use Progresso\Carbon_Fields\Service\REST_API_Service;
use Progresso\Carbon_Fields\Libraries\Sidebar_Manager\Sidebar_Manager;
use Progresso\Carbon_Fields\REST_API\Router as REST_API_Router;
use Progresso\Carbon_Fields\REST_API\Decorator as REST_API_Decorator;
use Progresso\Carbon_Fields\Event\Emitter;
use Progresso\Carbon_Fields\Event\PersistentListener;
use Progresso\Carbon_Fields\Event\SingleEventListener;
...
final class Carbon_Fields
{
    ...
    public static function resolve_with_arguments($identifier, $arguments, $subcontainer = null)
    {
        ...
        $container = new \Progresso\Carbon_Fields\Pimple\Container();
    ...
    public static function extend($class, $extender)
    {
        ...
        if (empty($extension_suffix)) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Could not determine "' . $class . '" extension type. Extension classes must have one of the following suffixes: ' . \implode(', ', \array_keys($type_dictionary)));
            return;
        }
        $identifier = \Progresso\Carbon_Fields\Helper\Helper::class_to_type($class, $extension_suffix);
     ...
     */
    public function install(\Progresso\Carbon_Fields\Pimple\Container $ioc)
    {
        $this->ioc = $ioc;
    }
    /**
    ...
    public static function boot()
    {
        ...
        do_action('progresso_carbon_fields_loaded');
    }
    /**
    ...
    {
        $register_action = 'progresso_carbon_fields_register_fields';
        $registered_action = 'progresso_carbon_fields_fields_registered';
        if (!doing_action($register_action) && !doing_action($registered_action) && did_action($registered_action) === 0) {
            \Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception::raise('Attempted to access a field before the ' . $register_action . ' and ' . $registered_action . ' actions have fired yet.');
        }
    }
    /**
    ...
    {
        $url = \Progresso\trailingslashit($directory);
        $count = 0;
        # Sanitize directory separator on Windows
        $url = \str_replace('\\', '/', $url);
        $possible_locations = array(
            WP_PLUGIN_DIR => \Progresso\plugins_url(),
            # If installed as a plugin
            WP_CONTENT_DIR => \Progresso\content_url(),
            # If anywhere in wp-content
            ABSPATH => \Progresso\site_url('/'),
        );
        foreach ($possible_locations as $test_dir => $test_url) {
            ...
            if ($count > 0) {
                return \Progresso\untrailingslashit($url);
            }
        }
        return '';
    ...
    {
        $ioc = new \Progresso\Carbon_Fields\Pimple\Container();
        $ioc['loader'] = function ($ioc) {
            return new \Progresso\Carbon_Fields\Loader\Loader($ioc['sidebar_manager'], $ioc['container_repository']);
        };
        $ioc['container_repository'] = function () {
            return new \Progresso\Carbon_Fields\Container\Repository();
        };
        ...
        $ioc['key_toolset'] = function () {
            return new \Progresso\Carbon_Fields\Toolset\Key_Toolset();
        };
        $ioc['wp_toolset'] = function () {
            return new \Progresso\Carbon_Fields\Toolset\WP_Toolset();
        };
        $ioc['sidebar_manager'] = function () {
            return new \Progresso\Carbon_Fields\Libraries\Sidebar_Manager\Sidebar_Manager();
        };
        $ioc['rest_api_router'] = function ($ioc) {
            return new \Progresso\Carbon_Fields\REST_API\Router($ioc['container_repository']);
        };
        $ioc['rest_api_decorator'] = function ($ioc) {
            return new \Progresso\Carbon_Fields\REST_API\Decorator($ioc['container_repository']);
        };
        /* Services */
        ...
        $ioc['services']['revisions'] = function () use($ioc) {
            return new \Progresso\Carbon_Fields\Service\Revisions_Service();
        };
        $ioc['services']['meta_query'] = function () use($ioc) {
            return new \Progresso\Carbon_Fields\Service\Meta_Query_Service($ioc['container_repository'], $ioc['key_toolset']);
        };
        $ioc['services']['legacy_storage'] = function () use($ioc) {
            return new \Progresso\Carbon_Fields\Service\Legacy_Storage_Service_v_1_5($ioc['container_repository'], $ioc['key_toolset']);
        };
        $ioc['services']['rest_api'] = function () use($ioc) {
            return new \Progresso\Carbon_Fields\Service\REST_API_Service($ioc['rest_api_router'], $ioc['rest_api_decorator']);
        };
        /* Events */
        $ioc['event_emitter'] = function () {
            return new \Progresso\Carbon_Fields\Event\Emitter();
        };
        $ioc['event_persistent_listener'] = function () {
            return new \Progresso\Carbon_Fields\Event\PersistentListener();
        };
        $ioc['event_single_event_listener'] = function () {
            return new \Progresso\Carbon_Fields\Event\SingleEventListener();
        };
        $ioc->register(new \Progresso\Carbon_Fields\Provider\Container_Condition_Provider());
        return $ioc;
    }
}
