...
namespace Progresso\Carbon_Fields\Loader;
...
class Loader
{
    ...
    public function __construct(\Progresso\Carbon_Fields\Libraries\Sidebar_Manager\Sidebar_Manager $sidebar_manager, \Progresso\Carbon_Fields\Container\Repository $container_repository)
    {
    ...
    public function boot()
    {
        ...
        include_once \Progresso\Carbon_Fields\DIR . '/core/functions.php';
        add_action('after_setup_theme', array($this, 'load_textdomain'), 9999);
        add_action('init', array($this, 'trigger_fields_register'), 0);
        add_action('progresso_carbon_fields_fields_registered', array($this, 'initialize_containers'));
        ...
        add_action('edit_form_after_title', array($this, 'add_progresso_carbon_fields_meta_box_contexts'));
        add_action('wp_ajax_progresso_carbon_fields_fetch_association_options', array($this, 'fetch_association_options'));
        # Enable the legacy storage service
        \Progresso\Carbon_Fields\Carbon_Fields::service('legacy_storage')->enable();
        # Enable the meta query service
        \Progresso\Carbon_Fields\Carbon_Fields::service('meta_query')->enable();
        # Enable the REST API service
        \Progresso\Carbon_Fields\Carbon_Fields::service('rest_api')->enable();
        # Enable post meta revisions service
        \Progresso\Carbon_Fields\Carbon_Fields::service('revisions')->enable();
    ...
    {
        $dir = \Progresso\Carbon_Fields\DIR . '/languages/';
    ...
    {
        try {
            do_action('progresso_carbon_fields_register_fields');
            do_action('progresso_carbon_fields_fields_registered');
        } catch (\Progresso\Carbon_Fields\Exception\Incorrect_Syntax_Exception $e) {
            $callback = '';
    ...
    protected function enqueue_style($src, $deps = array())
    {
        ...
        wp_enqueue_style('carbon-fields-' . $src, \Progresso\Carbon_Fields\URL . '/build/' . $context . '/' . $src . $suffix . '.css', $deps, \Progresso\Carbon_Fields\VERSION);
    }
    /**
    ...
    protected function enqueue_script($src, $deps = array())
    {
        $suffix = $this->get_assets_suffix();
        $context = $this->get_assets_context();
        wp_enqueue_script('carbon-fields-' . $src, \Progresso\Carbon_Fields\URL . '/build/' . $context . '/' . $src . $suffix . '.js', $deps, \Progresso\Carbon_Fields\VERSION);
    }
    /**
     * Enqueues the assets.
     *
    ...
    public function enqueue_assets()
    {
        ...
        $revisions = \Progresso\Carbon_Fields\Carbon_Fields::service('revisions');
        wp_localize_script('carbon-fields-vendor', 'cf', apply_filters('progresso_carbon_fields_config', array('config' => array('locale' => $this->get_ui_translations(), 'pagenow' => $pagenow, 'compactInput' => \Progresso\Carbon_Fields\COMPACT_INPUT, 'compactInputKey' => \Progresso\Carbon_Fields\COMPACT_INPUT_KEY, 'revisionsInputKey' => $revisions::CHANGE_KEY))));
    }
    /**
     ...
     */
    public function add_progresso_carbon_fields_meta_box_contexts()
    {
        global $post, $wp_meta_boxes;
        $context = 'progresso_carbon_fields_after_title';
    ...
    public function get_json_data()
    {
        ...
        foreach ($containers as $container) {
            ...
            if (\is_a($container, 'Progresso\\Carbon_Fields\\Container\\Block_Container', \true)) {
                $carbon_data['blocks'][] = $container_data;
            } else {
                $carbon_data['containers'][] = $container_data;
            }
        }
        $carbon_data['sidebars'] = \Progresso\Carbon_Fields\Helper\Helper::get_active_sidebars();
        return $carbon_data;
    }
    /**
    ...
    public function fetch_association_options()
    {
        ...
        $field = \Progresso\Carbon_Fields\Helper\Helper::get_field(null, $container_id, $field_name);
